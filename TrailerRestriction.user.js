// ==UserScript==
// @name         Trailer move restriction (DEBUG)
// @namespace    http://tampermonkey.net/
// @version      1.4.0-debug
// @description  DEBUG version – logs every step of Double Deck trailer restriction logic
...
(function () {
  ...
  // Any bay STARTING with these is restricted
  ...
  function normalize(str) {
    return String(str || '').toUpperCase().replace(/\s+/g, '').replace(/-/g, '');
  }
  ...
  function isRestrictedBay(bay) {
    const result = RESTRICTED_BAY_PREFIXES.some(p => bay.startsWith(p));
  ...
  function bindRestrictions(dialogRoot) {
    ...
    log('FINAL trailer type = Double Deck?', isDoubleDeck);
